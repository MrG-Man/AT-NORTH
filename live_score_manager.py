...
class LiveScoreManager:
    ...
    def __init__(self, data_manager_instance=None, cache_dir="cache"):
        ...
        self.min_update_interval = 300  # Minimum 5 minutes between checks
    ...
    def _should_update(self) -> bool:
        """
        Determine if we should perform an update based on timing, API limits, and business hours.
        ...
        # Check API usage limits (stop at 10 calls)
        usage_stats = self.live_api.get_usage_stats()
        if usage_stats['api_calls_used'] >= 10:  # Strict limit
            self.logger.warning("API usage limit reached (10 calls), skipping update")
            return False

        # Restrict updates to match hours only (09:00-17:00)
